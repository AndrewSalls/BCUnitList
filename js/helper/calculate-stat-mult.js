 ...
 */
export function getAbilityMult(fixedData, updatedData, targetTraits, statType) {
    const unitTargetTraits = getUnitTraitTargets(fixedData, updatedData);
    const traitTreasureMult = unitTargetTraits.filter(
        t => t in SETTINGS.traitEffectMult.trait && t in targetTraits
    ).map(
    ...
    for(const ability in Object.keys(SETTINGS.traitEffectMult[statType])) {
        if(hasAbility(ability, fixedData, updatedData)) {
            // Additional filter is because orbs that modify trait-specific abilities only work if unit targets that trait
            const orbAddition = getEffectOrb(ability, targetTraits, updatedData) // @ts-ignore Type hints do not detect that filter prevents null orbs from being reduced
            ...
            if(statType === "def") {
                multiplier *= Math.min(1, orbAddition);
 ...
 */
export function getUnitTraitTargets(fixedData, updatedData) {
    let targetTraits = new Set(fixedData.stats[updatedData.current_form].traits);    

    for(const trait of SETTINGS.traits) {
        if(updatedData.current_form >= 2) {
            const talentIndex = fixedData.talents.findIndex(t => t.name === `Target_${trait}`);
            if(talentIndex !== -1 && updatedData.talents[talentIndex] === 1) {
                targetTraits.add(trait);
            }
        }

        if(updatedData.current_form >= 3) {
            const talentIndex = fixedData.ultra_talents.findIndex(t => t.name === `Target_${trait}`);
            if(talentIndex !== -1 && updatedData.ultra_talents[talentIndex] === 1) {
                targetTraits.add(trait);
            }
        }
    }
 ...
 */
export function hasAbility(abilityName, fixedData, updatedData) {
    let foundAbility = fixedData.stats[updatedData.current_form].abilities.includes(abilityName);
    
    if(!foundAbility && updatedData.current_form >= 2) {
    ...
    if(!foundAbility && updatedData.current_form >= 3) {
...
export function getEffectOrb(ability, targetTraits, updatedData) {
    return updatedData.orb.filter(o =>
        ...
        ORB_MAP.traits[o.trait] in targetTraits &&
 ...
 */
export function getTalentStatMod(talentName, fixedData, updatedData) {
    let mult = 0;
